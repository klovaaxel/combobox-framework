...
export function handleComboBoxKeyPress(this: ComboboxFramework, event: KeyboardEvent): void {
    const isInputExpanded = () => this.input?.getAttribute("aria-expanded") === "true";
    ...
    switch (event.key) {
        ...
        case KeyCode.ArrowUp:
            ...
            if (isInputExpanded() && this.list) {
                this.focusItem(this.list?.children[this.list.children.length - 1] as HTMLElement);
            }
            event.preventDefault(); // prevent scrolling
            break;
        case KeyCode.Escape:
...
export function handleListKeyPress(this: ComboboxFramework, event: KeyboardEvent): void {
    ...
    switch (event.key) {
        ...
        case KeyCode.ArrowDown: {
            ...
            else if (this.list?.children) this.focusItem(this.list.children[0] as HTMLElement);
            event.preventDefault(); // prevent scrolling
            break;
        }
        case KeyCode.ArrowUp: {
