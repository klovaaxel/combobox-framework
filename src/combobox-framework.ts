...
export default class ComboboxFramework extends HTMLElement {
  ...
  public toggleList(
    newValue: boolean = this.input?.getAttribute('aria-expanded') !==
      true.toString()
  ): void {
  ...
  public selectItem(item: HTMLElement, grabFocus = true): void {
    ...
    if (this.input && item.dataset.display)
      this.input.value = item.dataset.display;
    ...
    else if (this.input && item.dataset.value)
  ...
  private searchList(openList = true, clearValue = true): void {
    ...
    searchedList = searchedList
      ...
      .sort(
        (a, b) =>
          a.score * (b.weight / a.weight) - b.score * (a.weight / b.weight)
      )
      .map((i) => ({
        item: i.item,
        score: i.score,
